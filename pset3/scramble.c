...
int main(int argc, string argv[])
{
    ...
    while (true)
    {
        ...
        if (s == NULL)
            break;
        // accept lowercase or uppercase characters
        else
...
bool lookup(string s)
{
    ...
    while (start <= end)
    {
        ...
        else if (strcmp(dictionary.words[mid].letters,s) > 0)
            end = mid - 1;
        else
            start = mid + 1;
    }
...
void scramble(void)
{
    ...
    // Place rotated chars in temp grid
    for (int i = 0; i < DIMENSION; i++)
        for (int j = DIMENSION - 1; j >= 0; j--)
            temp_grid[i][(DIMENSION - 1) - j] = grid[j][i];
